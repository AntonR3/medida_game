...
func _ready() -> void:
	$VBoxContainer/Collectibles.hide()
...
func show_collectibles():
	...
	for id in GlobalParamsGame.markers:
		...
		if score != null:
			...
			item.add_theme_constant_override("margin_left", 8)
			item.add_theme_constant_override("margin_right", 8)
			item.add_theme_constant_override("margin_top", 4)
			...
			label_score.size_flags_horizontal = Control.SIZE_SHRINK_CENTER
